...
class GameSelect(Menu):
    def __init__(self):
        modalities = ['position1', 'position2', 'color', 'image', 'audio', 'audio2', 'arithmetic']
        options = modalities[:]
...
class Visual:
    ...
    def spawnDual(self, position=0, position2=0,color=1, vis=0, number=-1, operation='none', variable = 0):
        ...
        #this is to add the small grid lines , divide by 3 to rescale
        ...
        self.color = (192, 192, 192)
        self.color8 = self.color * 8
        self.smallv_lines = batch.add(8, GL_LINES, None, ('v2i', (
                self.x1, self.y3,
                self.x2, self.y3,
                self.x1, self.y4,
                self.x2, self.y4,
        ...
        #this is the position for the resized sprite square
        self.square.x = self.center_x + ((field.size / 3)*((position2+1)%3 - 1) + (field.size / 3 - self.size)/2)/3
        self.square.y = self.center_y + ((field.size / 3)*((position2/3+1)%3 - 1) + (field.size / 3 - self.size)/2)/3
        self.square.x = self.square.x- field.size // 18 # might need to change to single slash
        self.square.y = self.square.y - field.size // 18 #might need to change to single slash


        # self.square.x = self.center_x - field.size // 6
        # self.square.y = self.center_y - field.size // 6
        self.square.scale = 1.0 * self.size / self.spr_square_size /3 #since size of square is 1/3
        self.square_size_scaled = self.square.width /3. #same here
        self.square.batch = batch
